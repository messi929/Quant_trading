...
### 🤔 미결 고민: 일간 신호 단일 사용의 적절성

**배경**: TWAP는 동일 신호를 시간 분산 집행하는 기법. 현재 구조는 KR/US 모두 하루 1회 신호를 생성하고 여러 Wave에 나눠 집행. 이 설계가 우리 목적에 맞는지에 대한 근본 의문.

**현재 구조**:
```
06:10  [US] 신호 생성 (NASDAQ 종가) → 23:40/02:00/04:30에 분할 집행
06:30  [KR] 신호 생성              → 09:10/11:00/13:30에 분할 집행
```

**의문의 핵심**: 신호 생성 후 수 시간이 지난 시점에도 같은 신호로 집행하는 것이 맞는가?

**고민 방향**:

1. **현재 유지 (daily signal + TWAP)**: 우리 모델이 daily OHLCV 기반이므로 하루 1신호가 자연스러운 단위. TWAP는 순수 집행 최적화. 인트라데이 노이즈 회피 효과도 있음.

2. **신호 유효기간 도입**: 시간이 지날수록 신호 확신도 감소 → Wave 비율 동적 조정 (Wave 1에 더 많이 집행, Wave 3은 신호 약해지면 스킵). 모델 score 임계값에 시간 decay 적용.

3. **인트라데이 모델 추가 (장기)**: 1시간봉/15분봉 데이터를 추가 학습하여 장 중 신호 갱신 가능하게. 현재 모델 아키텍처 전면 확장 필요.

**현재 판단**: 모델이 daily이므로 (1)이 맞는 선택. 단, dir_acc 52.5%인 현 모델 신뢰도를 감안하면 (2)의 "약한 신호는 Wave 3 스킵" 같은 가벼운 변형도 검토 가치 있음.
