...
서버 재기동 후 Wave 3에서 16건 정상 체결 확인.

---

## 24. Phase 17: 호가단위 라운딩 + 주문 안전장치 + MDD 계산 수정

**날짜**: 2026-03-13 | **배포**: 서버 22:09 KST

### 문제 진단

2026-03-13 실전 운용 로그 분석 결과 4가지 반복 장애 확인:

| 문제 | 증상 | 영향 |
|------|------|------|
| **호가단위 오류** | 089590, 008730, 336370, 322000 등 매 Wave 매도/매수 거부 (code=40030000) | Wave당 4~6건 실패, 전체 체결률 30% |
| **매매불가 종목** | 003060 모든 Wave에서 "매매불가 종목" (code=40070000) | 매 Wave 1건 실패 |
| **시세 0원 주문** | 204210, 140910 호가 조회 실패 → price=0으로 주문 → 500 Server Error | 매 Wave 1~2건 실패 |
| **MDD -100%** | `_compute_cumulative_mdd` 누적 계산에 portfolio_value=0 포함 | Sharpe/MDD 지표 왜곡 |

### 수정 1: KOSPI 호가단위 라운딩 (`utils/ticker_utils.py` 신규 함수)

KRX 호가 단위 규정에 따른 가격 라운딩 함수 추가:

```python
def kospi_tick_size(price: float) -> int:
    """KRX 호가 단위: <2K→1, <5K→5, <20K→10, <50K→50, <200K→100, <500K→500, ≥500K→1000"""

def round_to_tick(price: float, side: str = "buy") -> int:
    """매수=올림, 매도=내림 (체결 확률 최대화)"""
```

**적용 위치** (`live/signal_to_order.py`):

| 위치 | 적용 내용 |
|------|----------|
| `_get_limit_price()` | 가격 계산 후 `round_to_tick()` 적용 |
| `_submit_limit_order()` | 이중 방어 — 주문 직전 한번 더 라운딩 |
| `_execute_with_retry()` | 재주문 가격 조정(±0.5%) 후 라운딩 |

**검증 결과** (서버에서 실행):

| 종목 | 원래 가격 | 라운딩 후 | 틱 사이즈 |
|------|----------|----------|----------|
| 089590 | 5,274 | **5,270** | 10원 |
| 336370 | 8,364 | **8,360** | 10원 |
| 008730 | 24,077 | **24,050** | 50원 |
| 322000 | 126,718 | **126,700** | 100원 |
| 101140(매수) | 9,628 | **9,630** | 10원 |

### 수정 2: Sandbox 블랙리스트 (`live/signal_to_order.py`)
...
- `_compute_target_positions()`에서 `broker.mode == "sandbox"` 시 블랙리스트 종목 자동 스킵
- 실계좌 전환 시 영향 없음

### 수정 3: Price=0 주문 차단 (`live/signal_to_order.py`)

- `_submit_limit_order()`: `price <= 0` 시 `ValueError` 발생 → 500 Server Error 사전 방지
- 호가 라운딩 후에도 0원이면 추가 차단 (이중 방어)

### 수정 4: MDD -100% 계산 버그 (`tracking/trade_log.py`)

**원인**: 2026-02-24 `portfolio_value=0` 레코드 (설정 오류 시절) → MDD = (0 - 200M) / 200M = -100%

**수정**:
- `_compute_cumulative_mdd()`: `WHERE portfolio_value > 0` 필터 추가
- DB 정리: pv=0 레코드 삭제, 2026-03-06 이전 설정 오류 데이터(pv=200M) 삭제, 03-06 daily_return -49.49%→0.0 보정

### 부수 수정

- `execute_rebalance()` 매수 현금 체크: `get_domestic_price(order["ticker"])` → `get_domestic_price(kis_code(order["ticker"]))` — .KS suffix 누락 수정

### 수정 파일 요약

| 파일 | 변경 |
|------|------|
| `utils/ticker_utils.py` | `kospi_tick_size()`, `round_to_tick()` 추가 |
| `live/signal_to_order.py` | 호가 라운딩 4곳, 블랙리스트, price=0 방어, kis_code 누락 수정 |
| `tracking/trade_log.py` | MDD 계산에서 pv=0 제외 |
| DB (`trades.db`) | 불량 레코드 정리 (8행 삭제) |
