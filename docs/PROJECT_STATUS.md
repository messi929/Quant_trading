...
**백테스트 결과**: 1.94 (❌ 정규화 버그로 잘못된 수치, Phase 5에서 수정)

### Phase 5: 백테스트 버그 수정 + Walk-forward 검증 (2026-02-21)
**변경 파일**: `main.py`, `backtest/engine.py`

| 변경 항목 | 이전 | 이후 | 이유 |
|----------|------|------|------|
| 섹터 수익률 계산 | 정규화된 `return_1d` 사용 | close 가격에서 직접 pct_change 계산 | RobustScaler 정규화된 값은 실제 수익률 아님 |
| BacktestEngine max_position | 0.10 (10%/섹터) | 0.50 (50%/섹터) | top-3 신호(33%/섹터)가 클리핑으로 30%만 투자되던 문제 수정 |
| Walk-forward 실행 | 0 periods (windows 너무 큼) | 5 periods × 30일 | train_window=252 → 30으로 수정 |

**실제 백테스트 결과**: **Sharpe 0.70, MDD -7.79%, Return +7.02%** (기준선 대비 언더퍼폼)
...
> ⚠️ **중요**: Phase 1-4의 수치는 `return_1d` 정규화 버그로 인한 **잘못된 수치**임.
> Phase 5에서 버그 수정 후 실제 수익률 기반으로 재계산.

| 버전 | Sharpe | MDD | Return | Calmar | 날짜 | 비고 |
|------|--------|-----|--------|--------|------|------|
| 초기 | 0.25 | -36.4% | +14.3% | - | 2026-02-20 | ❌ 정규화 버그 |
| Phase 1 | 0.36 | -54.4% | +31.9% | 0.87 | 2026-02-20 | ❌ 정규화 버그 |
| Phase 2 | 0.62 | -25.9% | +29.9% | 1.71 | 2026-02-21 | ❌ 정규화 버그 |
| Phase 3 | 1.94 | -1.26% | +38.8% | 46.4 | 2026-02-21 | ❌ 정규화 버그 |
| Phase 4 | 1.94 | -1.26% | +38.8% | 46.4 | 2026-02-21 | ❌ 정규화 버그 |
| **Phase 5 (실제)** | **0.70** | **-7.79%** | **+7.02%** | **1.28** | 2026-02-21 | ✅ 실제 수익률 |
| Equal-weight 기준선 | 1.69 | -3.87% | +13.26% | - | 2026-02-21 | 비교 기준 |

**테스트 기간**: 180일 (2025-06-02 ~ 2026-02-18)
**초기 자본**: 1억원

### Walk-forward 결과 (Phase 5, 5×30일 기간)

| 기간 | Return | Sharpe | MDD |
|------|--------|--------|-----|
| 2025-07-16 ~ 08-26 | +0.06% | -0.19 | -4.44% |
| 2025-08-27 ~ 10-08 | -1.85% | -1.93 | -2.69% |
| 2025-10-09 ~ 11-19 | -3.77% | -2.27 | -3.93% |
| 2025-11-20 ~ 26-01-05 | +4.10% | +4.40 | -1.35% |
| 2026-01-06 ~ 02-18 | +2.15% | +1.79 | -2.18% |
| **평균** | **+0.14%** | **0.36** | **-2.92%** |

**결론**: 일관성 없음 (std=2.48 >> mean=0.36). 기간 4가 성과 대부분 차지.
